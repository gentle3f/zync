...
### Current catalog
The current mobile interest catalog on this branch contains **3,403 canonical entries** under the runtime parser semantics. An earlier ~2,032 raw-row estimate was incomplete because it missed entries generated by multiple `parseInterestFamily(...)` blocks.

Prompt expansion should target all release-stable canonical interests, not a hard-coded number. The architecture must remain compatible with future catalog growth.
