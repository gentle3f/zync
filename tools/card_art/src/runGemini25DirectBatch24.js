// Direct Google Gemini 2.5 Flash Image batch discriminator.
// IMPORTANT: this does NOT use fal.ai.
// It reuses the exact 24 semantic-anchor validation IDs/prompts for an apples-to-apples model/provider test.
//
// Usage:
...
const CFG_PATH = path.join(ROOT, 'catalog', 'gemini25_direct_batch_24_v1.json');
const OUT_DIR = path.join(ROOT, 'output', 'gemini25_direct_batch_24_v1');
...
function buildBatchPayload(rows) {
  return {
    batch: {
      display_name: 'zync-gemini25-direct-24-v1',
      input_config: {
        requests: {
...
async function collect(rows) {
  ...
  for (let i = 0; i < rows.length; i++) {
    ...
    const filename =
      ...
      '__gemini_2_5_flash_image__batch1.' + ext;
    ...
    entries.push({
      ...
      estimated_batch_image_output_cost_usd: 0.0195,
      image_written: true,
  ...
  const manifest = {
    version: 'gemini25_direct_batch_24_v1',
    generated_at: new Date().toISOString(),
    provider: 'Google Gemini API direct',
    model: MODEL,
    fal_ai: false,
    ...
    estimated_batch_image_output_cost_usd:
      Number((rows.length * 0.0195).toFixed(4)),
    note: 'Input token charges are additional. Prompts are compiled from the same current repo state as the FLUX comparison set.',
    entries,
  };
...
async function main() {
  ...
  if (args.has('--dry-run') || (!args.has('--submit') && !args.has('--collect'))) {
    console.log(
      'Dry run: 24 direct-Google Gemini 2.5 Flash Image prompts; ' +
      'Batch API estimated image-output cost=$' +
      config.expected.estimated_image_output_cost_usd.toFixed(3) +
      ' plus input tokens.',
    );
