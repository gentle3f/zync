// Zync Object-First V2.5 Wave-C Remediation Sentinel-8: AUTHORIZED
// targeted validation batch for exactly 3 failure classes fixed in v2.5
// (automotive brand-morphology, unsupported human-operated-object
// actions, severe semantic-routing ambiguity). Generates exactly the 8
// ids from the committed object_first_v25_wavec_remediation_sentinel8_v1.json
// config, all on the v2.5 production candidate.
//
// arts.illustration and transport.sports_cars were also repaired under
// v2.5 but are explicitly NOT part of this batch and must never appear
// here. transport.muscle_cars/pickup_trucks (automotive hold) and all
// genre/vocal music holds and wellness.stretching remain untouched.
//
// CRITICAL: this runner does NOT compile prompts. It loads the frozen
// final_compiled_prompt strings verbatim from
// output/object_first_v25_wavec_remediation_sentinel8_v1/frozen_prompts_v1.json
// and cross-checks each one's live SHA256 against the config's recorded
// prompt_sha256 (itself cross-verified against production_candidate_
// freeze_v2_5_v1.json and compiled_prompts_v1.jsonl at build time) before
// submission.
//
...
// No auto-chain into steady-state.
//
// Usage:
//   node src/runObjectFirstV2Sentinel8.js --dry-run
//   node src/runObjectFirstV2Sentinel8.js --submit
//   node src/runObjectFirstV2Sentinel8.js --collect
//
...
const CFG_PATH = path.join(CATALOG, 'object_first_v25_wavec_remediation_sentinel8_v1.json');
const FROZEN_PROMPTS_PATH = path.join(ROOT, 'output', 'object_first_v25_wavec_remediation_sentinel8_v1', 'frozen_prompts_v1.json');
...
function outputFilenameFor(id) {
  return `${id.replaceAll('.', '__')}__object_first_v25_wavec_remediation_sentinel8__gemini_3_1_flash_lite_image.jpg`;
}
...
function buildBatchPayload(rows) {
  return {
    batch: {
      display_name: 'zync-object-first-v25-wavec-remediation-sentinel8-v1',
      input_config: {
        requests: {
...
async function submit(rows) {
  ...
  console.log('Submitted direct Google Sentinel-8 batch:', result.name);
}
...
async function fetchStatusToScratchFile(statusUrl) {
  ...
  const scratchPath = path.join(os.tmpdir(), `zync-object-first-v25-sentinel8-status-${crypto.randomBytes(8).toString('hex')}.json`);
...
async function collect(rows) {
  ...
  try {
    ...
    const manifest = {
      version: 'object_first_v25_wavec_remediation_sentinel8_v1', generated_at: nowIso, provider: PROVIDER, model: MODEL, fal_ai: false,
...
async function main() {
  ...
  if (!Array.isArray(ids) || ids.length !== EXPECTED_COUNT || new Set(ids).size !== EXPECTED_COUNT) {
    throw new Error(`Sentinel-8 config must contain exactly ${EXPECTED_COUNT} unique hobby IDs.`);
  }
  if (JSON.stringify([...ids].sort()) !== JSON.stringify([...EXPECTED_IDS].sort())) {
    throw new Error(`Sentinel-8 config ids do not match the authorized list. Got: ${ids.join(', ')}`);
  }
  const FORBIDDEN = new Set(['arts.illustration', 'transport.sports_cars', 'transport.muscle_cars', 'transport.pickup_trucks', 'wellness.stretching', 'motorsport.cars']);
  for (const id of ids) if (FORBIDDEN.has(id)) throw new Error('Refusing forbidden/held/excluded canonical: ' + id);
  for (const c of config.cards) if (c.production_candidate_version !== 'v2.5') throw new Error(`Card ${c.id} is not on v2.5: ${c.production_candidate_version}`);
  ...
  if (args.has('--dry-run') || (!args.has('--submit') && !args.has('--collect'))) {
    console.log(`Dry run: ${EXPECTED_COUNT} direct-Google Sentinel-8 prompts (SHA256-verified, all v2.5); estimated cost=$${(EXPECTED_COUNT * BATCH_IMAGE_OUTPUT_PRICE_USD).toFixed(4)}.`);
    for (const row of rows) {
      console.log(`\n--- ${row.canonical_interest_id} / version=${row.production_candidate_version} / interaction_support_mode=${row.interaction_support_mode} / scene_family=${row.scene_family} / sha256=${row.prompt_sha256.slice(0, 16)}... ---`);
