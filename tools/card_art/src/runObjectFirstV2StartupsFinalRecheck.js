// Zync Object-First V2 Startups Final Single-Sentinel Recheck: AUTHORIZED
// paid execution of exactly one image (business.startups), testing the
// v2.3 narrow fix (hard zero-human containment against a product-holding
// hand/arm gesture, plus explicit blank/abstract packaging text control)
// built on top of Recheck-3 (commit 7425cce), which found the v2.2
// business-creation identity worked but leaked a human hand/arm holding
// a product and a readable invented product name on packaging.
//
// CRITICAL: this runner does NOT compile prompts. It loads the frozen
// final_compiled_prompt string verbatim from the config's recorded
// prompt_sha256 (computed from the real compiler output at build time,
// after the v2.3 rule-text edit) and cross-checks its live SHA256 before
// submission. Per explicit instruction, media.anime and learning.mock_trial
// are untouched by this task (verified unchanged before this run) and
// nothing about V2 beyond the single business.startups rule is touched -
// no rerolls, no mid-experiment fixes, no retry of a content-policy block.
//
// GENERATION ONLY: per the standing operating rule, this run performs no
// visual inspection of the collected image. Only technical/metadata
...
// runProductionBatch.js (a 1-request batch is small enough that this
// isn't strictly required, but it's kept for consistency with every
// other runner in this project).
//
// Usage:
//   node src/runObjectFirstV2StartupsFinalRecheck.js --dry-run
//   node src/runObjectFirstV2StartupsFinalRecheck.js --submit
//   node src/runObjectFirstV2StartupsFinalRecheck.js --collect
//
...
const CFG_PATH = path.join(CATALOG, 'object_first_v2_startups_final_recheck_v1.json');
const FROZEN_PROMPTS_PATH = path.join(ROOT, 'output', 'object_first_v2_startups_final_recheck_v1', 'frozen_prompt_v1.json');
const OUT_DIR = path.join(ROOT, 'output', 'object_first_v2_startups_final_recheck_v1');
...
function outputFilenameFor(id) {
  return `${id.replaceAll('.', '__')}__object_first_v2_startups_final_recheck__gemini_3_1_flash_lite_image.jpg`;
}

// Loads the 1 EXACT frozen prompt from the committed prompt file (never
// recompiled) and cross-checks its live SHA256 against the reference
// recorded in the config (catalog/object_first_v2_startups_final_recheck_v1.json,
// itself computed from the real compiler output at build time, after the
// v2.3 rule-text edit) - a mechanical guarantee that this run uses
// byte-identical text to what was statically audited.
...
function buildBatchPayload(rows) {
  return {
    batch: {
      display_name: 'zync-object-first-v2-startups-final-recheck-v1',
      input_config: {
        requests: {
...
async function fetchStatusToScratchFile(statusUrl) {
  ...
  const scratchPath = path.join(os.tmpdir(), `zync-object-first-v2-startups-final-status-${crypto.randomBytes(8).toString('hex')}.json`);
...
async function collect(rows) {
  ...
  try {
    ...
    const manifest = {
      version: 'object_first_v2_startups_final_recheck_v1', generated_at: nowIso, provider: PROVIDER, model: MODEL, fal_ai: false,
...
async function main() {
  ...
  if (!Array.isArray(ids) || ids.length !== 1 || ids[0] !== 'business.startups') {
    throw new Error('Object-first V2 startups-final-recheck config must contain exactly 1 hobby ID: business.startups.');
  }
  ...
  if (rows.length !== 1) throw new Error('Expected 1 frozen row, got ' + rows.length);

  if (args.has('--dry-run') || (!args.has('--submit') && !args.has('--collect'))) {
    console.log(`Dry run: 1 direct-Google business.startups prompt (frozen, SHA256-verified); estimated cost=$${(1 * BATCH_IMAGE_OUTPUT_PRICE_USD).toFixed(4)}.`);
    for (const row of rows) {
