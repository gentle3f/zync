// Zync Object-First V2 Sentinel-6: AUTHORIZED targeted production-
// remediation validation batch, run before Wave-C. Generates exactly the
// 6 ids from the committed object_first_v24_sentinel6_v1.json config -
// 4 on the frozen v2.4 candidate, 2 (music.singing, music.karaoke) on the
// narrow v2.4.1 repair. wellness.stretching is explicitly NOT part of
// this batch (temporary_generation_hold, semantic_repair_unvalidated).
//
// CRITICAL: this runner does NOT compile prompts. It loads the frozen
// final_compiled_prompt strings verbatim from
// output/object_first_v24_sentinel6_v1/frozen_prompts_v1.json and cross-
// checks each one's live SHA256 against the config's recorded
// prompt_sha256 (itself cross-verified against production_candidate_
// freeze_v2_4_v1.json / production_candidate_freeze_v2_4_1_v1.json per
// card's own version at build time) before submission.
//
...
// No auto-chain into Wave-C.
//
// Usage:
//   node src/runObjectFirstV2Sentinel6.js --dry-run
//   node src/runObjectFirstV2Sentinel6.js --submit
//   node src/runObjectFirstV2Sentinel6.js --collect
//
...
const CFG_PATH = path.join(CATALOG, 'object_first_v24_sentinel6_v1.json');
const FROZEN_PROMPTS_PATH = path.join(ROOT, 'output', 'object_first_v24_sentinel6_v1', 'frozen_prompts_v1.json');
...
function outputFilenameFor(id) {
  return `${id.replaceAll('.', '__')}__object_first_v24_sentinel6__gemini_3_1_flash_lite_image.jpg`;
}
...
function buildBatchPayload(rows) {
  return {
    batch: {
      display_name: 'zync-object-first-v24-sentinel6-v1',
      input_config: {
        requests: {
...
async function submit(rows) {
  ...
  console.log('Submitted direct Google Sentinel-6 batch:', result.name);
}
...
async function fetchStatusToScratchFile(statusUrl) {
  ...
  const scratchPath = path.join(os.tmpdir(), `zync-object-first-v24-sentinel6-status-${crypto.randomBytes(8).toString('hex')}.json`);
...
async function collect(rows) {
  ...
  try {
    ...
    const manifest = {
      version: 'object_first_v24_sentinel6_v1', generated_at: nowIso, provider: PROVIDER, model: MODEL, fal_ai: false,
...
async function main() {
  ...
  const EXPECTED_IDS = ['food.coffee', 'learning.fiction', 'music.singing', 'music.karaoke', 'learning.book_genre.booktube', 'technology.gadgets'];
  if (!Array.isArray(ids) || ids.length !== EXPECTED_COUNT || new Set(ids).size !== EXPECTED_COUNT) {
    throw new Error(`Sentinel-6 config must contain exactly ${EXPECTED_COUNT} unique hobby IDs.`);
  }
  if (JSON.stringify([...ids].sort()) !== JSON.stringify([...EXPECTED_IDS].sort())) {
    throw new Error(`Sentinel-6 config ids do not match the authorized list. Got: ${ids.join(', ')}`);
  }
  if (ids.includes('wellness.stretching')) throw new Error('wellness.stretching is on temporary_generation_hold and must NOT appear in Sentinel-6.');
  ...
  if (args.has('--dry-run') || (!args.has('--submit') && !args.has('--collect'))) {
    console.log(`Dry run: ${EXPECTED_COUNT} direct-Google Sentinel-6 prompts (SHA256-verified); estimated cost=$${(EXPECTED_COUNT * BATCH_IMAGE_OUTPUT_PRICE_USD).toFixed(4)}.`);
    for (const row of rows) {
