// Zync Object-First V2 PRODUCTION Wave-C-96: AUTHORIZED third real
// production wave, released from HOLD after Sentinel-6 PASS_REMEDIATION_VALIDATED
// (commit 1762230). Generates exactly the 96 NEW-production ids from the
// committed object_first_v2_production_waveC96_v1.json (selected from the
// pre-existing stratified wave_c_96 assignment, with 3 newly-held genre-
// music ids dropped and 4 deterministic steady-state replacements filled -
// see selectAndBuildWaveC96.js for the exact selection log).
//
// Per-card production candidate version is preserved exactly as recorded
// in the queue - 95 cards on v2.4, 1 card (music.piano) on the narrow
// v2.4.1 candidate. Never flattened to one version.
//
// Hard-excluded and refused if present: business.startups (reusable),
// music.rock/motorsport.cars (Canary-24 QA-quarantined), the 6 Sentinel-6
// remediation ids (already generated+approved), wellness.stretching
// (temporary_generation_hold), sports.american_football/technology.robotics
// (catalog quarantine).
//
// CRITICAL: this runner does NOT compile prompts. It loads the frozen
// final_compiled_prompt strings verbatim from
// output/object_first_v2_production_waveC96_v1/frozen_prompts_v1.json
// and cross-checks each one's live SHA256 against the config's recorded
// prompt_sha256 (itself cross-verified against the matching per-card
// freeze file - v2.4 or v2.4.1 - and compiled_prompts_v1.jsonl at build
...
//
// Usage:
//   node src/runObjectFirstV2ProductionWaveC96.js --dry-run
//   node src/runObjectFirstV2ProductionWaveC96.js --submit
//   node src/runObjectFirstV2ProductionWaveC96.js --collect
//
...
const CFG_PATH = path.join(CATALOG, 'object_first_v2_production_waveC96_v1.json');
const FROZEN_PROMPTS_PATH = path.join(ROOT, 'output', 'object_first_v2_production_waveC96_v1', 'frozen_prompts_v1.json');
...
function outputFilenameFor(id) {
  return `${id.replaceAll('.', '__')}__object_first_v2_production_wavec96__gemini_3_1_flash_lite_image.jpg`;
}
...
function buildBatchPayload(rows) {
  return {
    batch: {
      display_name: 'zync-object-first-v2-production-wavec96-v1',
      input_config: {
        requests: {
...
async function submit(rows) {
  ...
  console.log('Submitted direct Google production Wave-C-96 batch:', result.name);
}
...
async function fetchStatusToScratchFile(statusUrl) {
  ...
  const scratchPath = path.join(os.tmpdir(), `zync-object-first-v2-wavec96-status-${crypto.randomBytes(8).toString('hex')}.json`);
...
async function collect(rows) {
  ...
  try {
    ...
    const manifest = {
      version: 'object_first_v2_production_waveC96_v1', generated_at: nowIso, provider: PROVIDER, model: MODEL, fal_ai: false,
...
async function main() {
  ...
  if (!Array.isArray(ids) || ids.length !== EXPECTED_COUNT || new Set(ids).size !== EXPECTED_COUNT) {
    throw new Error(`Production Wave-C-96 config must contain exactly ${EXPECTED_COUNT} unique hobby IDs.`);
  }
  const FORBIDDEN = new Set(['business.startups', 'music.rock', 'motorsport.cars', 'sports.american_football', 'technology.robotics',
    'wellness.stretching', 'food.coffee', 'learning.fiction', 'music.singing', 'music.karaoke', 'learning.book_genre.booktube', 'technology.gadgets']);
  for (const id of ids) if (FORBIDDEN.has(id)) throw new Error('Refusing forbidden/quarantined/reusable/already-decided canonical: ' + id);
  ...
  for (const id of ids) {
    ...
    if (!row) throw new Error(`Wave-C id not found in production queue: ${id}`);
  ...
  if (args.has('--dry-run') || (!args.has('--submit') && !args.has('--collect'))) {
    console.log(`Dry run: ${EXPECTED_COUNT} direct-Google production Wave-C-96 prompts (SHA256-verified); estimated cost=$${(EXPECTED_COUNT * BATCH_IMAGE_OUTPUT_PRICE_USD).toFixed(4)}.`);
