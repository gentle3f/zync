// Targeted remediation validation runner: proves the exact Batch-1
// failure classes (business-category readable text, device-lid brand
// logos) are fixed, using the real production compiler with the new
// business_text_suppression_policy and strengthened
// global_brand_safety_policy. Direct Google Gemini Batch API only, no
// fal.ai. Reuses the permanent streaming-collection fix from
// runProductionBatch.js (a batch this size wouldn't normally need it, but
// the same code path is reused for consistency and safety).
//
// Usage:
//   node src/runRemediationTest12.js --dry-run
//   node src/runRemediationTest12.js --submit
//   node src/runRemediationTest12.js --collect
//
...
const CFG_PATH = path.join(CATALOG, 'remediation_test_12_v1.json');
const OUT_DIR = path.join(ROOT, 'output', 'remediation_test_12_v1');
...
function buildBatchPayload(rows) {
  return {
    batch: {
      display_name: 'zync-remediation-test-12-v1',
      input_config: {
        requests: {
...
async function submit(rows) {
  ...
  console.log('Submitted direct Google remediation-test batch:', result.name);
}
...
async function fetchStatusToScratchFile(statusUrl) {
  ...
  const scratchPath = path.join(os.tmpdir(), `zync-remediation-status-${crypto.randomBytes(8).toString('hex')}.json`);
...
async function collect(rows) {
  ...
  try {
    ...
    const manifest = {
      version: 'remediation_test_12_v1',
      generated_at: nowIso,
      provider: PROVIDER,
      model: MODEL,
      fal_ai: false,
...
async function main() {
  ...
  if (!Array.isArray(ids) || ids.length !== 12 || new Set(ids).size !== 12) {
    throw new Error('Remediation test config must contain exactly 12 unique hobby IDs.');
  }
  ...
  if (args.has('--dry-run') || (!args.has('--submit') && !args.has('--collect'))) {
    console.log(`Dry run: 12 direct-Google remediation-test prompts; estimated cost=$${(12 * BATCH_IMAGE_OUTPUT_PRICE_USD).toFixed(4)}.`);
    for (const row of rows) {
