// House-style A/B/C calibration: 6 hobbies x 3 isolated global-style-block
// candidates = 18 requests, via Gemini 3.1 Flash Lite Image direct (no fal.ai).
//
// IMPORTANT: this does NOT touch tools/card_art/specs/global_style_v1.json.
// The A/B/C style blocks are injected as an isolated in-memory globalStyle
...
//   node src/runStyleCalibration18.js --collect
//
...
const CFG_PATH = path.join(ROOT, 'catalog', 'style_calibration_18_v1.json');
const BLOCKS_PATH = path.join(ROOT, 'catalog', 'style_calibration_blocks_v1.json');
const OUT_DIR = path.join(ROOT, 'output', 'style_calibration_18_v1');
...
// constant across A/B/C - only visual style varies.
...
// A/B/C once the injected style block is stripped off. The style block is
// exactly `sections[0]` in buildPromptV1.js (globalStyle.prompt), and
...
// generic split. Throws before any paid call if a style swap ever leaked
...
function buildBatchPayload(rows) {
  return {
    batch: {
      display_name: 'zync-style-calibration-18-v1',
      input_config: {
        requests: {
...
// the on-disk forensic record stays small. Lesson carried over from the
// gemini-3.1-flash-lite 24-card run, where a key-name-only redaction
// missed thoughtSignature and produced a 65MB status file.
...
async function collect(rows) {
  ...
  const manifest = {
    version: 'style_calibration_18_v1',
    generated_at: new Date().toISOString(),
    provider: 'Google Gemini API direct',
    model: MODEL,
    fal_ai: false,
    ...
    note: 'Style A/B/C: only the injected global-style prompt block differs per row; semantic content verified identical pre-submission.',
    entries,
  };
...
async function main() {
  ...
  if (args.has('--dry-run') || (!args.has('--submit') && !args.has('--collect'))) {
    console.log(
      'Dry run: 18 direct-Google Gemini 3.1 Flash Lite style-calibration prompts (6 hobbies x 3 styles); ' +
