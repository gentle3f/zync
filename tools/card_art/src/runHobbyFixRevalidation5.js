// Narrow hobby-fix revalidation runner: proves the five new hobby-level
// overrides (business.fire_movement, arts.comedy_writing,
// arts.calligraphy, arts.zine_making, arts.hip_hop_dance) resolve the
// five remaining narrow Batch-1 blockers. Direct Google Gemini Batch API
// only, no fal.ai. Reuses the permanent streaming-collection fix from
// runProductionBatch.js.
//
// Usage:
//   node src/runHobbyFixRevalidation5.js --dry-run
//   node src/runHobbyFixRevalidation5.js --submit
//   node src/runHobbyFixRevalidation5.js --collect
//
...
const CFG_PATH = path.join(CATALOG, 'hobby_fix_revalidation_5_v1.json');
const OUT_DIR = path.join(ROOT, 'output', 'hobby_fix_revalidation_5_v1');
...
function buildBatchPayload(rows) {
  return {
    batch: {
      display_name: 'zync-hobby-fix-revalidation-5-v1',
      input_config: {
        requests: {
...
async function submit(rows) {
  ...
  console.log('Submitted direct Google hobby-fix-revalidation batch:', result.name);
}
...
async function fetchStatusToScratchFile(statusUrl) {
  ...
  const scratchPath = path.join(os.tmpdir(), `zync-hobby-fix-revalidation-status-${crypto.randomBytes(8).toString('hex')}.json`);
...
async function collect(rows) {
  ...
  try {
    ...
    const manifest = {
      version: 'hobby_fix_revalidation_5_v1',
      generated_at: nowIso,
      provider: PROVIDER,
      model: MODEL,
      fal_ai: false,
...
async function main() {
  ...
  if (!Array.isArray(ids) || ids.length !== 5 || new Set(ids).size !== 5) {
    throw new Error('Hobby-fix revalidation config must contain exactly 5 unique hobby IDs.');
  }
  ...
  if (rows.length !== 5) throw new Error('Expected 5 compiled rows, got ' + rows.length);

  if (args.has('--dry-run') || (!args.has('--submit') && !args.has('--collect'))) {
    console.log(`Dry run: 5 direct-Google hobby-fix-revalidation prompts; estimated cost=$${(5 * BATCH_IMAGE_OUTPUT_PRICE_USD).toFixed(4)}.`);
    for (const row of rows) {
