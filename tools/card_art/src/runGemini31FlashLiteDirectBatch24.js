// Direct Google Gemini 3.1 Flash Lite Image batch discriminator.
// IMPORTANT: this does NOT use fal.ai.
// Successor test to runGemini25DirectBatch24.js - reuses the exact same 24
// semantic-anchor validation IDs/prompts for an apples-to-apples model/provider test.
//
// Usage:
//   node src/runGemini31FlashLiteDirectBatch24.js --dry-run
//   node src/runGemini31FlashLiteDirectBatch24.js --submit
//   node src/runGemini31FlashLiteDirectBatch24.js --collect
//
...
const SRC_BATCH = path.join(ROOT, 'catalog', 'semantic_anchor_revalidation_24_v1.json');
const CFG_PATH = path.join(ROOT, 'catalog', 'gemini31_flash_lite_direct_batch_24_v1.json');
const OUT_DIR = path.join(ROOT, 'output', 'gemini31_flash_lite_direct_batch_24_v1');
...
function buildBatchPayload(rows) {
  return {
    batch: {
      display_name: 'zync-gemini31-flash-lite-direct-24-v1',
      input_config: {
        requests: {
...
// Deep-clones the status response but replaces any oversized string value
// (base64 inlineData image payloads, and this model's opaque
// thoughtSignature reasoning blobs) with just its length, so the on-disk
// forensic record stays small. Redacting by size rather than by a fixed
// key name ("data") is intentional: the prior gemini-2.5 run's raw
// status.json embedded all 24 images inline and ballooned to ~100MB in
// git history, and this model additionally returns ~1.3MB thoughtSignature
// strings per response that a key-name-only redaction missed entirely.
...
async function collect(rows) {
  ...
  // Write a redacted copy for forensics (canonical ID / index / metadata all
  // preserved) without embedding megabytes of base64 image data in git.
  ...
  for (let i = 0; i < rows.length; i++) {
    ...
    const filename =
      ...
      '__gemini_3_1_flash_lite_image__batch1.' + ext;
  ...
  const manifest = {
    version: 'gemini31_flash_lite_direct_batch_24_v1',
    generated_at: new Date().toISOString(),
    provider: 'Google Gemini API direct',
    model: MODEL,
    fal_ai: false,
    ...
    estimated_batch_image_output_cost_usd:
      Number((rows.length * 0.0168).toFixed(4)),
    note: 'Input token charges are additional. Prompts are compiled from the same current repo state as the FLUX and gemini-2.5-flash-image comparison sets.',
    entries,
  };
...
async function main() {
  ...
  const src = readJson(SRC_BATCH);
  const config = readJson(CFG_PATH);
  const ids = src.ids;
  if (!Array.isArray(ids) || ids.length !== 24 || new Set(ids).size !== 24) {
    throw new Error('Source semantic-anchor validation must contain 24 unique IDs.');
  }
  const rows = compileRows(ids);

  if (args.has('--dry-run') || (!args.has('--submit') && !args.has('--collect'))) {
    console.log(
      'Dry run: 24 direct-Google Gemini 3.1 Flash Lite Image prompts; ' +
