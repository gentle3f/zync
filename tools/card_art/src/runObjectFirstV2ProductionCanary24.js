// Zync Object-First V2 PRODUCTION Canary-24: AUTHORIZED first real
// production wave for the frozen v2.3 production candidate (source
// commit 9b031bc, rollout infrastructure commit 3ca31bd). Generates
// exactly the 24 NEW-production ids from the committed
// canary_24_manifest_v1.json - deliberately hard-stratified, not an
// easy sample. business.startups (the one existing accepted reusable
// asset) is explicitly excluded and not regenerated.
//
// CRITICAL: this runner does NOT compile prompts. It loads the frozen
// final_compiled_prompt strings verbatim from compiled_prompts_v1.jsonl
// and cross-checks each one's live SHA256 against BOTH the frozen
// production_candidate_freeze_v1.json AND production_queue_v2_object_first.json
// (already cross-verified consistent at config-build time) before
// submission, so what is sent to Gemini is guaranteed byte-identical to
// the frozen v2.3 architecture. Per explicit instruction, this run tests
// the frozen candidate - it does not fix anything discovered during
// execution, does not reroll, and does not auto-chain into Wave-B/C or
// steady-state.
//
...
//
// Usage:
//   node src/runObjectFirstV2ProductionCanary24.js --dry-run
//   node src/runObjectFirstV2ProductionCanary24.js --submit
//   node src/runObjectFirstV2ProductionCanary24.js --collect
//
...
const CFG_PATH = path.join(CATALOG, 'object_first_v2_production_canary24_v1.json');
const FROZEN_PROMPTS_PATH = path.join(ROOT, 'output', 'object_first_v2_production_canary24_v1', 'frozen_prompts_v1.json');
const OUT_DIR = path.join(ROOT, 'output', 'object_first_v2_production_canary24_v1');
...
function outputFilenameFor(id) {
  return `${id.replaceAll('.', '__')}__object_first_v2_production_canary24__gemini_3_1_flash_lite_image.jpg`;
}

// Loads the 24 EXACT frozen prompts from the committed prompt file (never
// recompiled) and cross-checks each one's live SHA256 against the
// reference recorded in the config (catalog/object_first_v2_production_canary24_v1.json,
// itself cross-verified consistent across compiled_prompts_v1.jsonl,
// production_candidate_freeze_v1.json, and production_queue_v2_object_first.json
// at build time) - a mechanical guarantee that this run uses
// byte-identical text to the frozen v2.3 production candidate.
...
function buildBatchPayload(rows) {
  return {
    batch: {
      display_name: 'zync-object-first-v2-production-canary24-v1',
      input_config: {
        requests: {
...
async function fetchStatusToScratchFile(statusUrl) {
  ...
  const scratchPath = path.join(os.tmpdir(), `zync-object-first-v2-canary24-status-${crypto.randomBytes(8).toString('hex')}.json`);
...
async function collect(rows) {
  ...
  try {
    ...
    const manifest = {
      version: 'object_first_v2_production_canary24_v1', generated_at: nowIso, provider: PROVIDER, model: MODEL, fal_ai: false,
...
async function main() {
  ...
  if (!Array.isArray(ids) || ids.length !== 24 || new Set(ids).size !== 24) {
    throw new Error('Production Canary-24 config must contain exactly 24 unique hobby IDs.');
  }
  if (ids.includes('business.startups')) {
    throw new Error('business.startups is the accepted reusable asset and must NOT appear in the Canary-24 generation list.');
  }
  ...
  if (rows.length !== 24) throw new Error('Expected 24 frozen rows, got ' + rows.length);

  if (args.has('--dry-run') || (!args.has('--submit') && !args.has('--collect'))) {
    console.log(`Dry run: 24 direct-Google production Canary-24 prompts (frozen v2.3, SHA256-verified); estimated cost=$${(24 * BATCH_IMAGE_OUTPUT_PRICE_USD).toFixed(4)}.`);
    for (const row of rows) {
