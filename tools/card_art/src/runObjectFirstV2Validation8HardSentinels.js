// Zync Object-First V2 Validation-8 (hard sentinels): AUTHORIZED paid
// execution of the 8-card hard-sentinel batch planned in
// catalog/object_first_v2_validation_8_plan_v1.json, with exact frozen
// prompts (post-scene_family-deconfliction) recorded in
// output/object_first_v2_full_catalog_audit_v1/validation_8_exact_prompts_v1.json
// and a SHA256 reference for each in
// catalog/object_first_v2_validation_8_hard_sentinels_v1.json, all at
// commit 2208ce3.
//
...
// prompt_sha256 (computed from the committed validation_8_exact_prompts_v1.json
...
// the architecture frozen at 2208ce3. Per explicit instruction, nothing
// about V2 (specs, compiler, routing, scene_family, text_mode,
// physical-logic domain) is touched by this run regardless of what it
// produces - no rerolls, no mid-experiment fixes.
//
...
// runProductionBatch.js (an 8-request batch is small enough that this
// isn't strictly required, but it's kept for consistency with every
// other runner in this project).
//
// Usage:
//   node src/runObjectFirstV2Validation8HardSentinels.js --dry-run
//   node src/runObjectFirstV2Validation8HardSentinels.js --submit
//   node src/runObjectFirstV2Validation8HardSentinels.js --collect
//
...
const CFG_PATH = path.join(CATALOG, 'object_first_v2_validation_8_hard_sentinels_v1.json');
const FROZEN_PROMPTS_PATH = path.join(ROOT, 'output', 'object_first_v2_full_catalog_audit_v1', 'validation_8_exact_prompts_v1.json');
const OUT_DIR = path.join(ROOT, 'output', 'object_first_v2_validation_8_hard_sentinels_v1');
...
function outputFilenameFor(id) {
  return `${id.replaceAll('.', '__')}__object_first_v2_val8_hard_sentinels__gemini_3_1_flash_lite_image.jpg`;
}

// Loads the 8 EXACT frozen prompts from the committed prompt-audit output
// (never recompiled) and cross-checks each one's live SHA256 against the
// reference recorded in the config (catalog/object_first_v2_validation_8_hard_sentinels_v1.json,
// itself computed from the committed validation_8_exact_prompts_v1.json
// at build time) - a mechanical guarantee that this run uses
// byte-identical text to the architecture frozen at 2208ce3.
...
function buildBatchPayload(rows) {
  return {
    batch: {
      display_name: 'zync-object-first-v2-validation-8-hard-sentinels-v1',
      input_config: {
        requests: {
...
async function fetchStatusToScratchFile(statusUrl) {
  ...
  const scratchPath = path.join(os.tmpdir(), `zync-object-first-v2-val8hs-status-${crypto.randomBytes(8).toString('hex')}.json`);
...
async function collect(rows) {
  ...
  try {
    ...
    const manifest = {
      version: 'object_first_v2_validation_8_hard_sentinels_v1', generated_at: nowIso, provider: PROVIDER, model: MODEL, fal_ai: false,
...
async function main() {
  ...
  if (!Array.isArray(ids) || ids.length !== 8 || new Set(ids).size !== 8) {
    throw new Error('Object-first V2 validation-8 hard-sentinels config must contain exactly 8 unique hobby IDs.');
  }
  const EXPECTED_IDS = [
    'transport.modelrailways', 'food.japanese', 'media.anime', 'business.startups',
    'music.pop', 'learning.model_united_nations', 'business.marketing', 'learning.mock_trial',
  ];
  for (const id of EXPECTED_IDS) if (!ids.includes(id)) throw new Error(`Config is missing required hard-sentinel id: ${id}`);
  ...
  if (rows.length !== 8) throw new Error('Expected 8 frozen rows, got ' + rows.length);

  if (args.has('--dry-run') || (!args.has('--submit') && !args.has('--collect'))) {
    console.log(`Dry run: 8 direct-Google object-first-v2-validation-8-hard-sentinels prompts (frozen, SHA256-verified against 2208ce3); estimated cost=$${(8 * BATCH_IMAGE_OUTPUT_PRICE_USD).toFixed(4)}.`);
    for (const row of rows) {
