// House-style D1/D2/D3 calibration: 6 hobbies x 3 isolated Japanese
// collectible-anime global-style-block candidates = 18 requests, via
// Gemini 3.1 Flash Lite Image direct (no fal.ai). Follows the A/B/C round
// (removed the stock-photo look, but none of A/B/C landed the intended
// Japanese commercial-anime/collectible-card energy).
//
// IMPORTANT: this does NOT touch tools/card_art/specs/global_style_v1.json.
// The D1/D2/D3 style blocks are injected as an isolated in-memory
// globalStyle clone at compile time only, for this experiment run.
//
// Usage:
//   node src/runStyleCalibrationD18.js --dry-run
//   node src/runStyleCalibrationD18.js --submit
//   node src/runStyleCalibrationD18.js --collect
//
...
const CFG_PATH = path.join(ROOT, 'catalog', 'style_calibration_d_18_v1.json');
const BLOCKS_PATH = path.join(ROOT, 'catalog', 'style_calibration_d_blocks_v1.json');
const OUT_DIR = path.join(ROOT, 'output', 'style_calibration_d_18_v1');
...
// constant across D1/D2/D3 - only visual style varies.
...
// Verifies that, for each hobby, the compiled prompt is identical across
// D1/D2/D3 once the injected style block is stripped off. The style block
// is exactly `sections[0]` in buildPromptV1.js (globalStyle.prompt), and
// sections are joined with "\n\n" - so compiledPrompt always starts with
// the exact style-block string followed by "\n\n" + the rest. We strip
// that known-length prefix rather than naively splitting the whole
// compiled string on "\n\n", because the style-block prose itself
// contains internal "\n\n" line breaks that would otherwise misalign a
// generic split (lesson carried over from the A/B/C round). Throws before
// any paid call if a style swap ever leaked into semantic content.
function verifySemanticIdentityAcrossStyles(rows, ids, styleVersions, blocks) {
  for (const id of ids) {
    const remainders = styleVersions.map(sv => {
      const row = rows.find(r => r.canonical_interest_id === id && r.style_version === sv);
      const stylePrompt = blocks.styles[sv].prompt;
      if (!row.final_prompt.startsWith(stylePrompt)) {
        throw new Error('Compiled prompt for ' + id + '/' + sv + ' does not start with its injected style block as expected.');
      }
      return row.final_prompt.slice(stylePrompt.length);
    });
    const [first, ...rest] = remainders;
    for (let i = 0; i < rest.length; i++) {
      if (rest[i] !== first) {
        throw new Error(
          'Semantic-identity check failed for ' + id + ': style ' +
          styleVersions[i + 1] + ' differs from style ' + styleVersions[0] +
          ' outside the injected style block.',
        );
      }
    }
  }
  console.log('Semantic-identity check passed: all ' + ids.length + ' hobbies compile identically across ' + styleVersions.join('/') + ' once the style block is stripped.');
}

function buildBatchPayload(rows) {
  return {
    batch: {
      display_name: 'zync-style-calibration-d-18-v1',
      input_config: {
        requests: {
...
async function collect(rows) {
  ...
  const manifest = {
    version: 'style_calibration_d_18_v1',
    generated_at: new Date().toISOString(),
    provider: 'Google Gemini API direct',
    model: MODEL,
    fal_ai: false,
    ...
    note: 'Style D1/D2/D3: only the injected global-style prompt block differs per row; semantic content verified identical pre-submission.',
    entries,
  };
...
async function main() {
  ...
  if (!Array.isArray(styleVersions) || styleVersions.length !== 3) {
    throw new Error('Style calibration config must contain exactly 3 style versions.');
  }
  const rows = compileRows(ids, styleVersions, blocks);
  if (rows.length !== 18) throw new Error('Expected 18 compiled rows, got ' + rows.length);

  verifySemanticIdentityAcrossStyles(rows, ids, styleVersions, blocks);

  if (args.has('--dry-run') || (!args.has('--submit') && !args.has('--collect'))) {
    console.log(
      'Dry run: 18 direct-Google Gemini 3.1 Flash Lite D1/D2/D3 style-calibration prompts (6 hobbies x 3 styles); ' +
