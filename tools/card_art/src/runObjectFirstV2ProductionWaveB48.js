// Zync Object-First V2 PRODUCTION Wave-B-48: AUTHORIZED second real
// production wave for the frozen v2.3 production candidate (source
// commit 9b031bc). Generates exactly the 48 NEW-production ids from the
// committed object_first_v2_production_waveB48_v1.json (selected from
// the pre-existing stratified wave_b_48 assignment, with music.pop
// dropped for the genre-music temporary hold and deterministically
// replaced by collecting.stamps - see applyProductionRolloutV2Updates.js).
//
// Hard-excluded and refused if present: business.startups (reusable
// asset), music.rock / motorsport.cars (Canary-24 QA-quarantined), any
// id currently under generation_hold_reason "music_genre_semantic_repair".
//
// CRITICAL: this runner does NOT compile prompts. It loads the frozen
// final_compiled_prompt strings verbatim from
// output/object_first_v2_production_waveB48_v1/frozen_prompts_v1.json
// and cross-checks each one's live SHA256 against the config's recorded
// prompt_sha256 (itself cross-verified against production_candidate_freeze_v1.json
// and compiled_prompts_v1.jsonl at build time) before submission.
//
...
// No auto-chain into Wave-C or steady-state.
//
// Usage:
//   node src/runObjectFirstV2ProductionWaveB48.js --dry-run
//   node src/runObjectFirstV2ProductionWaveB48.js --submit
//   node src/runObjectFirstV2ProductionWaveB48.js --collect
//
...
const CFG_PATH = path.join(CATALOG, 'object_first_v2_production_waveB48_v1.json');
const FROZEN_PROMPTS_PATH = path.join(ROOT, 'output', 'object_first_v2_production_waveB48_v1', 'frozen_prompts_v1.json');
...
function outputFilenameFor(id) {
  return `${id.replaceAll('.', '__')}__object_first_v2_production_waveb48__gemini_3_1_flash_lite_image.jpg`;
}
...
function buildBatchPayload(rows) {
  return {
    batch: {
      display_name: 'zync-object-first-v2-production-waveb48-v1',
      input_config: {
        requests: {
...
async function submit(rows) {
  ...
  console.log('Submitted direct Google production Wave-B-48 batch:', result.name);
}
...
async function fetchStatusToScratchFile(statusUrl) {
  ...
  const scratchPath = path.join(os.tmpdir(), `zync-object-first-v2-waveb48-status-${crypto.randomBytes(8).toString('hex')}.json`);
...
async function collect(rows) {
  ...
  try {
    ...
    const manifest = {
      version: 'object_first_v2_production_waveB48_v1', generated_at: nowIso, provider: PROVIDER, model: MODEL, fal_ai: false,
...
async function main() {
  ...
  if (!Array.isArray(ids) || ids.length !== EXPECTED_COUNT || new Set(ids).size !== EXPECTED_COUNT) {
    throw new Error(`Production Wave-B-48 config must contain exactly ${EXPECTED_COUNT} unique hobby IDs.`);
  }
  const FORBIDDEN = new Set(['business.startups', 'music.rock', 'motorsport.cars', 'sports.american_football', 'technology.robotics']);
  for (const id of ids) if (FORBIDDEN.has(id)) throw new Error('Refusing forbidden/quarantined/reusable canonical: ' + id);
  ...
  for (const id of ids) {
    ...
    if (!row) throw new Error(`Wave-B id not found in production queue: ${id}`);
  ...
  if (args.has('--dry-run') || (!args.has('--submit') && !args.has('--collect'))) {
    console.log(`Dry run: ${EXPECTED_COUNT} direct-Google production Wave-B-48 prompts (frozen v2.3, SHA256-verified); estimated cost=$${(EXPECTED_COUNT * BATCH_IMAGE_OUTPUT_PRICE_USD).toFixed(4)}.`);
    for (const row of rows) {
