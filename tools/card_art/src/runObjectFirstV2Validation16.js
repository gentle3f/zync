// Zync Object-First V2 Validation-16: AUTHORIZED paid execution of the
// 16-card validation batch planned in
// catalog/object_first_v2_validation_16_plan_v1.json and prompt-audited
// (exact prompts + SHA256) in
// output/object_first_v2_validation_16_prompt_audit_v1/compiled_prompts_v1.json
// at commit 4eba93c.
//
// CRITICAL: this runner does NOT compile prompts. It loads the frozen
// final_compiled_prompt strings verbatim from that audit output file and
// cross-checks each one's SHA256 against the recorded prompt_sha256
// before submission, so what is sent to Gemini is guaranteed
// byte-identical to what was statically audited - no recompilation, no
// manual edits, no drift from the architecture frozen at 4eba93c. Per
// explicit instruction, nothing about V2 (specs, compiler, routing) is
// touched by this run regardless of what it produces - no rerolls, no
...
// runProductionBatch.js (a 16-request batch is small enough that this
// isn't strictly required, but it's kept for consistency with every
// other runner in this project).
//
// Usage:
//   node src/runObjectFirstV2Validation16.js --dry-run
//   node src/runObjectFirstV2Validation16.js --submit
//   node src/runObjectFirstV2Validation16.js --collect
//
...
const CFG_PATH = path.join(CATALOG, 'object_first_v2_validation_16_v1.json');
const FROZEN_PROMPTS_PATH = path.join(ROOT, 'output', 'object_first_v2_validation_16_prompt_audit_v1', 'compiled_prompts_v1.json');
const FROZEN_PLAN_PATH = path.join(CATALOG, 'object_first_v2_validation_16_plan_v1.json');
const OUT_DIR = path.join(ROOT, 'output', 'object_first_v2_validation_16_v1');
...
function outputFilenameFor(id) {
  return `${id.replaceAll('.', '__')}__object_first_v2_val16__gemini_3_1_flash_lite_image.jpg`;
}

// Loads the 16 EXACT frozen prompts from the committed prompt audit
// (never recompiled) and cross-checks each one's live SHA256 against the
// recorded prompt_sha256 - a mechanical guarantee that this run uses
// byte-identical text to what was statically audited at 4eba93c.
function loadFrozenRows(ids) {
  const frozen = readJson(FROZEN_PROMPTS_PATH);
  const plan = readJson(FROZEN_PLAN_PATH);
  const frozenById = new Map(frozen.map(f => [f.canonical_interest_id, f]));
  const planById = new Map(plan.cards.map(c => [c.id, c]));

  return ids.map((id, index) => {
    const frozenEntry = frozenById.get(id);
    if (!frozenEntry) throw new Error(`No frozen compiled prompt found for "${id}" in ${FROZEN_PROMPTS_PATH}`);
    const planEntry = planById.get(id);
    if (!planEntry) throw new Error(`No plan entry found for "${id}" in ${FROZEN_PLAN_PATH}`);
    const liveSha = sha256Text(frozenEntry.final_compiled_prompt);
    if (liveSha !== frozenEntry.prompt_sha256) {
      throw new Error(`SHA256 mismatch for "${id}": recorded=${frozenEntry.prompt_sha256} recomputed=${liveSha}. Refusing to submit a prompt that may have drifted from the audited text.`);
    }
    return {
      ...
      protagonist_type: planEntry.protagonist_type,
      object_first_status: planEntry.object_first_status,
      structural_containment_rule_id: planEntry.structural_containment_rule_id,
      composition_archetype: planEntry.composition_archetype,
      palette_lighting_route: planEntry.palette_lighting_route,
      effect_level: planEntry.effect_level,
      final_prompt: frozenEntry.final_compiled_prompt,
      prompt_sha256: frozenEntry.prompt_sha256,
      model: MODEL,
      provider: PROVIDER,
      request_index: index,
      request_key: id,
...
function buildBatchPayload(rows) {
  return {
    batch: {
      display_name: 'zync-object-first-v2-validation-16-v1',
      input_config: {
        requests: {
...
async function fetchStatusToScratchFile(statusUrl) {
  ...
  const scratchPath = path.join(os.tmpdir(), `zync-object-first-v2-val16-status-${crypto.randomBytes(8).toString('hex')}.json`);
...
async function collect(rows) {
  ...
  try {
    ...
    const manifest = {
      version: 'object_first_v2_validation_16_v1', generated_at: nowIso, provider: PROVIDER, model: MODEL, fal_ai: false,
...
async function main() {
  ...
  if (!Array.isArray(ids) || ids.length !== 16 || new Set(ids).size !== 16) {
    throw new Error('Object-first V2 validation-16 config must contain exactly 16 unique hobby IDs.');
  }
  ...
  const rows = loadFrozenRows(ids);
  if (rows.length !== 16) throw new Error('Expected 16 frozen rows, got ' + rows.length);

  if (args.has('--dry-run') || (!args.has('--submit') && !args.has('--collect'))) {
    console.log(`Dry run: 16 direct-Google object-first-v2-validation-16 prompts (frozen, SHA256-verified against 4eba93c); estimated cost=$${(16 * BATCH_IMAGE_OUTPUT_PRICE_USD).toFixed(4)}.`);
    for (const row of rows) {
      console.log(`\n--- ${row.title} [${row.canonical_interest_id}] / ${row.archetype} / sha256=${row.prompt_sha256.slice(0, 16)}... ---`);
