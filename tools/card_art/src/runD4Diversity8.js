// D4 diversity stress test: 8 hobbies via the REAL production compiler,
// with the new deterministic diversity layer active (diversityProfiles
// passed to compileHobbyPrompt) plus the 3 localized recipe fixes
// (books.reading text leak, wellness.cold_plunge cold-vs-warm drift,
// sports.skateboarding footwear silhouette), through Gemini 3.1 Flash Lite
// Image direct (no fal.ai). D4 itself (global_style_v1.json's prompt
// field) is NOT touched by this round.
//
// Usage:
//   node src/runD4Diversity8.js --dry-run
//   node src/runD4Diversity8.js --submit
//   node src/runD4Diversity8.js --collect
//
...
const CFG_PATH = path.join(ROOT, 'catalog', 'style_d4_diversity_8_v1.json');
const OUT_DIR = path.join(ROOT, 'output', 'style_d4_diversity_8_v1');
...
function compileRows(ids) {
  ...
  for (const id of ids) {
    ...
    const compiled = compileHobbyPrompt({
      ...
      diversityProfiles: ctx.diversityProfiles,
    });
    rows.push({
...
function buildBatchPayload(rows) {
  return {
    batch: {
      display_name: 'zync-d4-diversity-8-v1',
      input_config: {
        requests: {
...
async function collect(rows) {
  ...
  const manifest = {
    version: 'style_d4_diversity_8_v1',
    generated_at: new Date().toISOString(),
    provider: 'Google Gemini API direct',
    model: MODEL,
    fal_ai: false,
    ...
    note: 'D4 diversity stress test using the real production compiler with the new deterministic diversity layer active, plus the 3 localized recipe fixes (books.reading, wellness.cold_plunge, sports.skateboarding).',
    entries,
  };
...
async function main() {
  ...
  if (!Array.isArray(ids) || ids.length !== 8 || new Set(ids).size !== 8) {
    throw new Error('D4 diversity config must contain exactly 8 unique hobby IDs.');
  }
  ...
  if (args.has('--dry-run') || (!args.has('--submit') && !args.has('--collect'))) {
    console.log(
      'Dry run: 8 direct-Google Gemini 3.1 Flash Lite D4-diversity prompts (real production compiler); ' +
    ...
    for (const row of rows) {
      console.log(
        ...
        row.archetype + ' / ' + row.visual_variant + ' / diversity=' +
        JSON.stringify(row.diversity_profile) + ' ---\n' +
        row.final_prompt,
      );
    }
    return;
