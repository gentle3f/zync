...
5. Added localization generation and all eight initial UI locale files.
...
- Latest meaningful CI-tested code head: `38c9f7526debef41659478699828ff942438ff7b` (later `.gitignore` commit does not change build inputs).
- GitHub Actions run: `35131801550` (`Zync V1 CI`, run #3).
- At last check the job was IN PROGRESS in Flutter setup; compile/analyze results were not yet available.
- Earlier queued/running CI jobs may be cancelled by the new concurrency rule.
- Do not claim the app builds until run #3 (or a newer run) reaches analyze/test/build successfully.

## Still missing / next active task
1. Inspect CI run #3. If it fails, read exact job logs, fix only evidenced compile/config errors, rerun until green.
