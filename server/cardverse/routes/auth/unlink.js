...
import { unlinkIdentityFromAccount } from '../../account_lifecycle.js';
import { getCardverseDatabase } from '../../db.js';
import { verifyProviderAccountAction } from '../../provider_auth.js';
import {
...
function statusFor(error) {
  ...
  if (error?.code === 'cardverse_last_identity_unlink_forbidden') return 409;
  return 500;
}
...
export default async function handler(req, res) {
  ...
  try {
    ...
    const result = await unlinkIdentityFromAccount(db, session.accountId, identity);
    ...
    if (status >= 500) console.error('Cardverse identity unlink failed', error?.code || error?.message);
    return res.status(status).json({ error: error?.code || 'cardverse_identity_unlink_failed' });
  }
}
